...
function TokenWarningCard({ token }: TokenWarningCardProps) {
  ...
  const duplicateNameOrSymbol = useMemo(() => {
    ...
    return Object.keys(allTokens).some(tokenAddress => {
      ...
      console.error('userToken.equals(token) -> ', userToken.equals(token))
    ...
    <Wrapper error={duplicateNameOrSymbol}>
      <AutoRow gap="6px">
        ...
        <AutoColumn gap="10px" justify="flex-start">
          ...
          {chainId && (
            <ExternalLink style={{ fontWeight: 400 }} href={getBSCscanLink(token.address, 'token')}>
    ...
    <Modal isOpen={isOpen} onDismiss={handleDismiss} maxHeight={90}>
      <WarningContainer className="token-warning-container">
        <AutoColumn gap="lg">
          <AutoRow gap="6px">
            <StyledWarningIcon />
            <TYPE.main color={'primary5'}>Token imported</TYPE.main>
          </AutoRow>
          <TYPE.body color={'text6'}>
            Anyone can create an ERC20 token on BSC with <em>any</em> name, including creating fake versions of existing
          ...
          <RowBetween>
            ...
            <ButtonConfirmed
              ...
              altDisabledStyle={!understandChecked}
              // error={true}
              width={'140px'}
              padding="0.5rem 1rem"
